...
def main():
    ...
    images_to_show = sorted(images_to_show, key=functools.cmp_to_key(images_cmp))
    bug_ids = set([file_name.split('_')[0] for file_name in images_to_show])
    ordered_images = []
    for bug_id in bug_ids:
        ordered_images.extend([file_name for file_name in images_to_show if bug_id == file_name.split('_')[0]])
